...
def finalize_context_engine_compression_notification(agent: Any, *, committed: bool) -> bool:
    """Emit or discard deferred boundary observers; repeated calls are no-ops."""
    lifecycle_pending = getattr(agent, _PENDING_COMPRESSION_LIFECYCLE_FINISH, None)
...
def _candidate_rejected(
    agent: Any, compressed: Any, messages: list, messages_before_compression: Optional[list], *,
    live_messages_before_compression: Optional[list],
    ...
    if compressed == messages_before_compression or (
        ...
        rollback_messages = (
            live_messages_before_compression or messages_before_compression or messages
        )
        if messages != rollback_messages:
    ...
    if not _compressor_attempt_is_current(agent.context_compressor, attempt_generation):
        ...
        _restore_messages_snapshot(
            messages, live_messages_before_compression or messages_before_compression
        )
        agent._last_compaction_in_place = False
...
def _commit_compaction(
    ...
    live_messages_before_compression: Optional[list],
    made_progress: bool, attempt: _Attempt,
) -> _CommitOutcome:
    ...
    if agent._session_db:
        ...
        try:
            ...
            agent.commit_memory_session(messages_before_compression or messages)
...
class _SummaryPhase:
    ...
    live_messages_before_compression: Optional[list] = None
...
def _run_summary_phase(
    ...
    attempt: _Attempt, fork_lifecycle: Any = None,
) -> _SummaryPhase:
    ...
    try:
        ...
        if not in_place:
            ...
            if _adopted_parent is not None:
                ...
                pre_msg_count = len(messages)
                if fork_lifecycle is not None:
                    fork_lifecycle.rematerialize_after_adoption(messages)
        ...
        # Persisted non-user runtime context is already model-visible through active
        # history. It must not be summarized, re-ingested by a context engine, or
        # handed to a memory provider as human transcript. Keep the live list intact
        # for rollback and Request Fork fidelity; every compression consumer receives
        # the same private filtered copy.
        ...
        memory_context = _pre_compress_memory_context(
            agent, compression_input, checkpoint_required
        )
        if fork_lifecycle is not None:
            fork_lifecycle.start(compression_input)
        ...
        compressed = _run_summary_dispatch(
            agent, compression_input, compress_fn, compress_kwargs, commit_fence=commit_fence,
        ...
        if fork_lifecycle is not None:
            fork_lifecycle.set_outcome("aborted", "explicit_interrupt")
            fork_lifecycle.finish()
        lease.release()
        _emit_aborted_attempt_telemetry(
        ...
        if fork_lifecycle is not None:
            fork_lifecycle.set_outcome("failed", type(_compress_exc).__name__)
            fork_lifecycle.finish()
        lease.release()
    ...
    return _SummaryPhase(
        ...
        live_messages_before_compression=live_messages_before_compression,
...
def _begin_compression_attempt(agent: Any, *, force: bool, defer_notification: bool) -> _Attempt:
    ...
    if defer_notification and (
        callable(getattr(agent, _PENDING_CONTEXT_ENGINE_NOTIFICATION, None))
        or callable(getattr(agent, _PENDING_COMPRESSION_LIFECYCLE_FINISH, None))
    ):
        raise RuntimeError("a compression notification is already pending")
    ...
    return _Attempt(snapshot, generation, started_at, attempt_id)
...
class _ForkCompressionLifecycle:
    ...
    def __init__(
        ...
        self.request_fork = request_fork
...
def compress_context(
    ...
    request_fork_rematerializer: Optional[Any] = None,
) -> Tuple[list, str]:
    ...
    fork_lifecycle = _ForkCompressionLifecycle(
        agent,
        ...
        defer_finish=defer_context_engine_notification,
    )
    ...
    live_messages_before_compression = phase.live_messages_before_compression
    ...
    try:
        ...
        if _candidate_rejected(
            ...
            if getattr(agent.context_compressor, "_last_compress_aborted", False):
                rejection_reason = "summary_generation_aborted"
            elif not compressed:
                rejection_reason = "empty_transcript"
            elif compressed == messages_before_compression or (
                ...
                rejection_reason = "attempt_superseded"
            fork_lifecycle.set_outcome("aborted", rejection_reason)
            return messages, _existing_system_prompt(agent, system_message)
        persistent_compression_context = fork_lifecycle.prepare_persistent_context()
        if commit_fence is not None:
            ...
            if not _commit_fence_entered:
                ...
                fork_lifecycle.set_outcome("aborted", "commit_fence_cancelled")
                return messages, _existing_sp
        ...
        if commit.refused_prompt is not None:
            fork_lifecycle.set_outcome("aborted", "compression_would_grow")
        ...
        fork_lifecycle.set_outcome(
            lifecycle.commit_status,
            "compression_committed" if lifecycle.commit_status == "committed" else "session_split_failed",
        )
        _emit_compression_attempt_telemetry(
