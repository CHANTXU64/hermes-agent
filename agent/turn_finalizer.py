...
def _resolve_budget_fallback(
    ...
    current_turn_user_idx=None, _ext_prefetch_cache="", _plugin_user_context="",
    _plugin_request_context="",
) -> Tuple[Any, Any, bool]:
    ...
    if (
        ...
        if _pending_verification_response:
            ...
            if any(
                ...
                if key != "current_turn_user_idx"
            ):
                final_response = agent._handle_max_iterations(
                    messages, api_call_count, **summary_context
                )
            else:
                # Preserve compatibility with small embedders/test doubles that
                # implement the historical two-argument callback.
...
def finalize_turn(
    ...
    from agent.conversation_loop import logger
    from fork_features.request_context import strip_legacy_api_content
    ...
    final_response, _turn_exit_reason, preserved_verification_fallback = _resolve_budget_fallback(
        ...
        _plugin_request_context=_plugin_request_context,
        logger=logger,
    )
    ...
    agent._sync_external_memory_for_turn(
        ...
        interrupted=interrupted, messages=messages, turn_id=turn_id,
    )
