...
class MemoryManager:
    ...
    def __init__(self, *, external_prefetch_timeout: Optional[float] = None) -> None:
        ...
        timeout = _EXTERNAL_PREFETCH_TIMEOUT_S if external_prefetch_timeout is None else float(external_prefetch_timeout)
        if timeout <= 0:
    ...
    def _provider_prefetch_timeout(self, provider: MemoryProvider) -> float:
        """Resolve the outer wait budget without overriding explicit callers."""
        if self._external_prefetch_timeout_explicit:
            return self._external_prefetch_timeout

        timeout_resolver = getattr(provider, "prefetch_timeout_seconds", None)
        if not callable(timeout_resolver):
            return self._external_prefetch_timeout
        try:
            declared_timeout: Any = timeout_resolver()
        except Exception as exc:
            logger.warning(
                "Memory provider '%s' failed to resolve its prefetch timeout; "
                "using %.1fs: %s",
                provider.name,
        ...
        try:
            ...
            logger.warning(
                "Memory provider '%s' declared a non-convertible prefetch timeout "
                "of type %s; using %.1fs: %s",
                provider.name,
                type(declared_timeout).__name__,
                self._external_prefetch_timeout,
                exc,
            )
            return self._external_prefetch_timeout
        if not math.isfinite(timeout) or timeout <= 0:
            logger.warning(
                "Memory provider '%s' declared invalid prefetch timeout %r; "
                "using %.1fs",
                provider.name,
                timeout,
                self._external_prefetch_timeout,
    ...
    def prefetch_all(self, query: str, *, session_id: str = "", turn_id: str = "",
        ...
        parts = self._each_provider(
            ...
            lambda p: self._prefetch_provider(
                p, clean_query, session_id=session_id, turn_id=turn_id,
    ...
    def _prefetch_provider(
        ...
        turn_id: str = "", previous_assistant_message: str = "",
    ) -> str:
        ...
        prefetch_kwargs: Dict[str, Any] = {"session_id": session_id}
        params = _signature_params(provider.prefetch)
        if params is None or _has_var_kwargs(params) or "previous_assistant_message" in params:
            prefetch_kwargs["previous_assistant_message"] = previous_assistant_message
        if params is None or _has_var_kwargs(params) or "turn_id" in params:
            prefetch_kwargs["turn_id"] = turn_id
        if provider.name == "builtin":
            return provider.prefetch(query, **prefetch_kwargs)

        result_box: Dict[str, Any] = {}

        def _run() -> None:
            try:
                result_box["value"] = provider.prefetch(query, **prefetch_kwargs) or ""
        ...
        timeout = self._provider_prefetch_timeout(provider)
        thread.join(timeout)
        if thread.is_alive():
            logger.warning(
            ...
            timeout_hook = getattr(provider, "on_prefetch_timeout", None)
            if callable(timeout_hook):
                with suppress(Exception):
                    timeout_hook(session_id=session_id, turn_id=turn_id)
            return ""
        ...
        for provider in self._providers:
            try:
                provider.on_session_rewind(
                    session_id, turns_undone=turns_undone, **kwargs,
                )
            except Exception as e:
                logger.debug("Memory provider '%s' on_session_rewind failed: %s", provider.name, e)

    def queue_prefetch_all(self, query: str, *, session_id: str = "", turn_id: str = "") -> None:
        ...
        def _queue(provider: MemoryProvider) -> None:
            params = _signature_params(provider.queue_prefetch)
        ...
        self._submit_background(lambda: self._each_provider(
            "queue_prefetch failed (non-fatal)", _queue, providers=providers,
        ), kind="prefetch")
