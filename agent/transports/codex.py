...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        if is_codex_backend:
            ...
            def _fork_bound_key(value: Any) -> Optional[str]:
                if not isinstance(value, str):
                    return None
                return _bounded_prompt_cache_key(value)

            routing_overrides = dict(params.get("request_overrides") or {})
            if "prompt_cache_key" in params and "prompt_cache_key" not in routing_overrides:
                routing_overrides["prompt_cache_key"] = params.get("prompt_cache_key")
            kwargs = apply_codex_backend_cache_routing(
                kwargs,
                session_id=str(session_id or ""),
                request_overrides=routing_overrides,
                fallback_cache_key=cache_key,
                bound_key=_fork_bound_key,
            )
