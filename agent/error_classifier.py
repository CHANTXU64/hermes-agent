...
# Ambiguous disconnects (no status): always a retryable transport timeout.
# Session size is not evidence of provider context overflow.
...
def _by_transport(c: _Ctx) -> Optional[Verdict]:
    ...
    if any(p in msg for p in (
        ...
        "without sending complete message",
    )) and not c.status_code:
        return _V_TIMEOUT
    # Status-less disconnects stay transport timeouts. Session size is not
    # evidence of provider context overflow (fork contract).
