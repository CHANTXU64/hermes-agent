...
def handle_api_error(
    ...
    current_turn_user_idx: Any, _physical_request_capture: Any,
) -> ApiErrorVerdict:
    ...
    capture = _physical_request_capture if isinstance(_physical_request_capture, dict) else {}
    request_fork = capture.get("request_fork")
    request_fork_rematerializer = capture.get("request_fork_rematerializer")
    if "request_fork" not in capture:
        # Defensive fallback for an exception raised before the final physical
    ...
    _ov = recover_from_overflow(
        ...
        api_messages=api_messages, api_kwargs=api_kwargs, system_message=system_message,
        active_system_prompt=active_system_prompt, conversation_history=conversation_history,
