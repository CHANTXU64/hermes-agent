...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    if messages and messages[-1].get("role") == "assistant":
        ...
        if not visible:
            ...
            # Keep the transcript hidden and empty, but give the current
            # request projection a non-empty neutral assistant turn so the
            # pre-call sanitizer does not re-heal this row on every call.
            from agent.agent_runtime_helpers import _INTERRUPTED_PLACEHOLDER

            placeholder["api_content"] = _INTERRUPTED_PLACEHOLDER
            placeholder["_request_only_api_content"] = True
...
def _consume_request_only_api_content(api_msg: Dict[str, Any]) -> Optional[str]:
    """Move an active-turn sidecar onto the provider copy, then strip it.

    Legacy persisted ``api_content`` is deliberately ignored. Only redirect
    rows carrying the in-memory request-only marker may replace clean content;
    neither field is valid provider metadata.
    """
    request_only = api_msg.pop("_request_only_api_content", False) is True
    api_content = api_msg.pop("api_content", None)
    if request_only and isinstance(api_content, str) and api_content:
        api_msg["content"] = api_content
        return api_content
    return None


def _strip_request_only_api_content(messages: List[Dict[str, Any]]) -> None:
    """Remove provider-only sidecars after the turn's final request completes."""
    for message in messages:
        if not isinstance(message, dict):
            continue
        message.pop("api_content", None)
        message.pop("_request_only_api_content", None)


def _messages_for_turn_result(
    messages: List[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    """Return a turn result's messages after removing request-only fields.

    ``run_conversation`` has terminal branches that return before the normal
    finalizer. Keep those branches on the same persistence boundary instead
    of letting an active-turn redirect sidecar escape through an early result.
    """
...
def _content_policy_blocked_result(
    ...
    *,
    ...
    return {
        ...
        "messages": _messages_for_turn_result(messages),
        "api_calls": api_call_count,
        "completed": False,
        "failed": True,
...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        for idx, msg in enumerate(messages):
            ...
            # Live redirect checkpoints use api_content as request-only replay
            # text while keeping transcript content clean. Legacy persisted
            # sidecars carry no marker and are dropped instead of replayed.
            _api_content = _consume_request_only_api_content(api_msg)
        ...
        if agent._use_prompt_caching and agent.provider != "moa":
            ...
            request_api_messages_for_estimate = _initial_cache_plan.messages
        ...
        if agent.provider == "moa":
            ...
            if _moa_prepared_request is not None:
                request_api_messages_for_estimate = _moa_prepared_request["messages"]
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                request_api_messages, _moa_prepared_request, tools_for_api = (
                ...
                if finish_reason == "length":
                    ...
                    if len(messages) > 1:
                        ...
                        return {
                            ...
                            "messages": _messages_for_turn_result(rolled_back_messages),
                            "api_calls": api_call_count,
                            "completed": False,
                            "partial": True,
