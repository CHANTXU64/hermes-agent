...
def _request_fork_defer_enabled(agent: Any) -> bool:
    """Whether compaction must wait for the provider-ready parent request."""
    try:
        from fork_features.request_fork import compression_request_fork_enabled

        return bool(compression_request_fork_enabled(agent))
    except Exception:
        return False
...
def _idle_compaction(
    ...
    if not _tc._should_idle_compact(
        ...
        return
    if _request_fork_defer_enabled(agent):
        agent._compression_request_fork_pending = True
        logger.info(
            "Deferring request-fork-aware idle compaction until the exact API "
            "request snapshot is available (session %s)",
...
def _preflight_compression(
    ...
    if _should_compress_now and _request_fork_defer_enabled(agent):
        agent._compression_request_fork_pending = True
        _request_fork_preflight_deferred = True
        _should_compress_now = False
        logger.info(
            "Deferring request-fork-aware preflight compression until the exact "
            "API request snapshot is available (session %s)",
            agent.session_id or "none",
        )
    if _should_compress_now:
        ...
        # Sub-threshold and unblocked — re-arm only when no provider-ready
        # request capture is waiting to run.
        if not _request_fork_preflight_deferred:
            _clear_overflow_warn(agent)
        # Engine maintenance only when NO skip-branch fired: cooldown, deferred
        # estimate, native Codex, or request-fork capture keep it unconsulted.
        if not (
