...
def _fork_smart_approval_scope(messages: list):
    """Bind current-turn user/clarify context for Fork Smart Approval."""
    token = None
    reset_fn = None
    try:
        ...
        ctx = build_smart_approval_context(messages)
        token = set_smart_approval_context(ctx)
        reset_fn = reset_smart_approval_context
    except Exception:
        logger.debug("Fork smart approval context bind failed", exc_info=True)
    try:
        yield
    finally:
...
def build_smart_approval_context(messages: list) -> dict[str, Any]:
    ...
    return build_policy_context(
        messages,
        is_real_user_message=_is_real_user_message,
        real_user_message_text=_real_user_message_text,
        strip_stale_todo_snapshot=_strip_stale_todo_snapshot,
    )
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    with _fork_smart_approval_scope(messages):
        _execute_tool_calls_concurrent_inner(
            agent, assistant_message, messages, effective_task_id, api_call_count, finalize=finalize
        )


def _execute_tool_calls_concurrent_inner(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    """Execute tool calls concurrently; results are appended in original call order.
    ``finalize=False`` skips end-of-batch budget enforcement and /steer injection (the
    segmented dispatcher owns turn-end work)."""
...
def _execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    owns turn-end work)."""
    with _fork_smart_approval_scope(messages):
        _execute_tool_calls_sequential_inner(
            agent, assistant_message, messages, effective_task_id, api_call_count, finalize=finalize
        )


def _execute_tool_calls_sequential_inner(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    """Execute tool calls sequentially (single calls or interactive tools). ``finalize=False``
    skips end-of-batch budget enforcement and /steer injection (the segmented dispatcher
    owns turn-end work)."""
    _tool_budget = _budget_for_agent(agent)  # once per turn, not per result
