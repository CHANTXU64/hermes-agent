...
def perform_api_call(
    ...
    interrupted: Any, messages: Any, current_turn_user_idx: Any,
    _physical_request_capture: Any,
) -> ApiCallVerdict:
    """Issue the request (see ``_should_stream`` for the streaming decision)."""
    response = None
    if isinstance(_physical_request_capture, dict):
    ...
    def _capture_physical_request(physical_api_kwargs):
        ...
        try:
            ...
            _physical_request_capture.update(
                api_kwargs=copy.deepcopy(physical_api_kwargs),
                request_fork=frozen,
                request_fork_rematerializer=rematerializer,
            )
        except Exception:
            _physical_request_capture.clear()
            logger.warning(
