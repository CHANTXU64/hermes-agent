...
def _wire_message_shadow(msg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    * Explicitly marked request-only ``api_content`` SUBSTITUTES ``content``: only a
      non-empty STRING sidecar on a user/assistant row displaces content. Legacy
      persisted sidecars and the marker itself are not provider bytes.
    ...
    * Opaque provider blobs (``encrypted_content`` on codex reasoning / compaction items) are
    ...
    for k, v in msg.items():
        if (
            k in ("_anthropic_content_blocks", "reasoning_details", "_request_only_api_content")
