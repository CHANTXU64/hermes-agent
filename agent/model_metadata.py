...
def _wire_message_shadow(msg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    * A marked request-only ``api_content`` is a SUBSTITUTE for ``content``, not
      an addition to it. ``conversation_loop._consume_request_only_api_content``
      pops both marker and sidecar from the API copy and overwrites ``content``.
      Unmarked legacy sidecars are dropped, so exactly one clean/wire value is
      counted and stale persisted bytes cannot trigger premature compaction.

      The substitution mirrors that helper's guard exactly: the request-only
      marker must be present and the sidecar must be a non-empty STRING on a
      ``user``/``assistant`` row. Any other sidecar shape is popped and discarded
      on the wire without touching ``content``, so a shadow that substituted
      unconditionally would UNDERcount those rows — the dangerous direction,
      since it makes compaction fire too late and the turn dies on a hard context
      error.
    * Base64 image payloads are replaced with a placeholder; they are charged
    ...
    sidecar_wins = (
        msg.get("_request_only_api_content") is True
