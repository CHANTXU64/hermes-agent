...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        provider_reported_model = (
            getattr(final, "provider_reported_model", None)
            or getattr(final, "model", None)
        )
        ...
        for _entry in get_plugin_auxiliary_tasks():
            if _entry.get("key") == task:
                ...
                if isinstance(_defaults, dict):
                    merged = {**_defaults, **task_config}
                    configured_provider = str(task_config.get("provider") or "").strip()
                    default_provider = str(_defaults.get("provider") or "").strip()
                    if configured_provider and configured_provider != default_provider:
                        # Provider-specific routing defaults must not follow an
                        # explicit switch to another backend.
                        for key in ("model", "base_url", "api_key", "extra_body"):
