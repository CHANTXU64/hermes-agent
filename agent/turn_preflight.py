...
def run_preflight_compression(
    ...
    _request_fork_pending = bool(getattr(agent, "_compression_request_fork_pending", False))
    _eligible = (
    ...
    if _eligible and _request_fork_pending:
        agent._compression_request_fork_pending = True
        # The pending fork can only be frozen after the provider request has
        # been fully assembled.  Fall through to that assembly in the current
        # loop iteration; restarting here would observe the same pending bit
        # forever and never reach the consumer.
...
def compress_after_tool_results(
    ...
    if _post_tool_compression_requested:
        try:
            ...
            _request_fork_post_tool_defer = compression_request_fork_enabled(agent)
        except Exception:
            _request_fork_post_tool_defer = False
        if _request_fork_post_tool_defer:
            agent._compression_request_fork_pending = True
            logger.info(
                "Deferring request-fork-aware post-tool compression until the exact "
