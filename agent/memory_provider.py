...
class MemoryProvider(ABC):
    ...
    def prefetch_timeout_seconds(self) -> Optional[float]:
        """Optional provider-specific bound for one synchronous prefetch.

        ``None`` keeps the MemoryManager's configured external-provider timeout.
        Providers with a multi-stage bounded pipeline may override this.
        """
        return None
    ...
    def on_session_rewind(self, session_id: str, *, turns_undone: int = 1, **kwargs) -> None:
        """Same session id, but /undo removed recent turns from the transcript."""
