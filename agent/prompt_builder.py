...
def build_memory_guidance(
    ...
    """Gate the Fork governance guidance by independently enabled stores."""
    if not memory_enabled and not profile_enabled:
        return ""
    if memory_enabled and profile_enabled:
        return MEMORY_GUIDANCE
    if memory_enabled:
        target_limit = (
            "The persistent user profile store is disabled; use only "
            "target='memory' for built-in memory operations. "
        )
    else:
        target_limit = (
            "The built-in notes store is disabled; use only target='user' for "
            "built-in memory operations. "
        )
    return target_limit + MEMORY_GUIDANCE
