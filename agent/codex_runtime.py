...
def run_codex_stream(
    agent, api_kwargs: dict, client: Any = None, on_first_delta=None,
    ...
    def _open_codex_stream(next_api_kwargs: dict[str, Any]):
        ...
        if callable(on_physical_request):
            try:
                on_physical_request(copy.deepcopy(stream_kwargs))
            except Exception:
                logger.warning(
                    "Codex physical-request observer failed; continuing provider call",
                    exc_info=True,
                )
