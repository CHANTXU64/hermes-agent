...
class GatewaySessionCommandsMixin:
    ...
    async def _handle_undo_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            rewind = getattr(mm, "on_session_rewind", None)
            if callable(rewind):
                rewind(session_entry.session_id, turns_undone=result["turns_undone"])
        except Exception as e:
            logger.debug("undo: memory rewind notify skipped: %s", e)
        try:
            self._evict_cached_agent(session_key)
        except Exception as e:
    ...
    async def _run_manual_compression(self, source, session_entry, history: list, request) -> str:
        ...
        try:
            ...
            request_fork = build_out_of_turn_compression_request_snapshot(tmp_agent, msgs)
