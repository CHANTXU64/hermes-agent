...
from gateway.session import (
    SessionSource, build_session_key, is_shared_multi_user_session, neutralize_untrusted_inline_text
)
...
class GatewayInboundMixin:
    ...
    async def _hm_clarify_reply(
        ...
        _attachment_notes = []
        for _path in getattr(event, "media_urls", None) or []:
            _display_name, _agent_path = self._inbound_attachment_display_name(_path)
            _attachment_notes.append(
                f"[The user sent an attachment: '{_display_name}'. "
                f"It is saved at: {_agent_path}]"
            )
        _response_context = "\n".join(_attachment_notes)
        ...
        _text_outcome = _clarify_mod.TEXT_NO_PENDING

        def _resolve_text_response(session_key, response, *, response_context=None):
            nonlocal _text_outcome
            _text_outcome = _clarify_mod.attempt_text_response_for_session(
                session_key, response, response_context=response_context,
            )
            return _text_outcome == _clarify_mod.TEXT_RESOLVED

        _disposition = resolve_pending_clarify_reply(
            session_key=_quick_key,
            response_text=_raw_clarify_reply,
            response_context=_response_context,
            has_audio=_clarify_has_audio,
            resolve_text_response=_resolve_text_response,
        )
        if _disposition is ClarifyReplyDisposition.RETAIN_PENDING:
            return _retain("voice transcription produced no usable text")
        if _disposition is ClarifyReplyDisposition.RESOLVED:
            logger.info(
            ...
            return ""
        # Slash commands and empty non-attachment events bypass Clarify.
    ...
    async def _hm_run_exec_quick_command(self, command: str, exec_cmd: str, qcmd: dict, source: SessionSource) -> str:
        """Run a ``type: exec`` quick command in the gateway process (30 s cap, sanitized env).

        Session identity is per-subprocess and only when ``session_env: true``.
        """
        try:
            ...
            if qcmd.get("session_env") is True:
                ...
                try:
                    quick_session_id = peek(key) if peek else None
                    if asyncio.iscoroutine(quick_session_id):
                        quick_session_id = await quick_session_id
                except Exception as exc:
                    logger.warning(
                ...
                if not quick_session_id:
                    return f"Quick command '/{command}' requires an active Hermes session."
                sanitized_env["HERMES_SESSION_ID"] = str(quick_session_id)
    ...
    async def _hm_dispatch_quick_and_plugin_commands(
        ...
        if qcmd is not None:
            ...
            if qtype == "exec":
                ...
                return True, await self._hm_run_exec_quick_command(command, exec_cmd, qcmd, source), command
            if qtype != "alias":
