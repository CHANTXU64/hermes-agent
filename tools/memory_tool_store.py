...
class MemoryStore:
    ...
    def path_for_target(self, target: str) -> Path:
        """Public Store seam for resolving one built-in memory target."""
        return self._path_for(target)

    def read_target_entries_checked(self, target: str) -> Tuple[List[str], bool]:
        """Public Store seam for a checked live target read."""
        return self._read_entries_checked(self._path_for(target))

    @staticmethod
    def preview_entries(entries: List[str], width: int = 80) -> List[str]:
        """Public Store seam for bounded ambiguous-match feedback."""
        return [e[:width] + ("..." if len(e) > width else "") for e in entries]

    @staticmethod
    def read_entries(path: Path) -> List[str]:
        """Public Store seam used only for an audit baseline inventory."""
        return MemoryStore._read_file(path)

    @staticmethod
    def read_entries_checked(path: Path) -> Tuple[List[str], bool]:
        """Public Store seam for safe background-review snapshots."""
        return MemoryStore._read_entries_checked(path)

    @staticmethod
    def sanitize_entries_for_snapshot(entries: List[str], filename: str) -> List[str]:
        """Public Store seam for threat-scanned background-review snapshots."""
        from tools.threat_patterns import scan_for_threats
    ...
    @contextmanager
    def transaction(self, target: str, coordinator_path: Path):
        """Hold coordinator then target locks for one external transaction policy."""
        with self._file_lock(coordinator_path):
            path = self._path_for(target)
            with self._file_lock(path):
                yield MemoryStoreTransaction(self, target, path)
    ...
    def _batch_failure(self, target: str, message: str) -> Dict[str, Any]:
        """Batch-abort failure without echoing unchanged live entries."""
        return self._consolidation_failure(
            _error(message + " No operations were applied (batch is all-or-nothing).", usage=self._usage(target)))

    def _mutate(self, target: str, mutate, *, skip_drift: bool = False, _lock_held: bool = False) -> Dict[str, Any]:
    ...
    def add(self, target: str, content: str, *, _lock_held: bool = False) -> Dict[str, Any]:
        ...
        return self._mutate(target, _add, skip_drift=True, _lock_held=_lock_held)

    def replace(self, target: str, old_text: str, new_content: str, *, _lock_held: bool = False) -> Dict[str, Any]:
        """Find the entry containing old_text (whole-entry exact match first) and
        replace the whole entry; old_text only locates it and is not patched in place."""
        ...
        if scan_error := _scan_memory_content(new_content):
            return _error(scan_error)
        return self._edit(target, old_text.strip(), new_content, _lock_held=_lock_held)

    def remove(self, target: str, old_text: str, *, _lock_held: bool = False) -> Dict[str, Any]:
        """Remove the entry containing old_text substring."""
        if not old_text.strip():
            return _error("old_text cannot be empty.")
        return self._edit(target, old_text.strip(), None, _lock_held=_lock_held)

    def _edit(self, target: str, old_text: str, new_content: Optional[str], *, _lock_held: bool = False) -> Dict[str, Any]:
        ...
        return self._mutate(target, _apply, _lock_held=_lock_held)
    ...
    def apply_batch(self, target: str, operations: List[Dict[str, Any]], *, _lock_held: bool = False) -> Dict[str, Any]:
...
class MemoryStoreTransaction:
    """Stable Store layer for a lock-held mutation transaction."""

    def __init__(self, store: MemoryStore, target: str, path: Path) -> None:
        self._store = store
        self._target = target
        self.path = path

    def read_checked(self) -> Tuple[List[str], bool]:
        return MemoryStore._read_entries_checked(self.path)

    def apply(self, operations: List[Dict[str, Any]], *, batch: bool = False) -> Dict[str, Any]:
        if not batch and len(operations) == 1:
            operation = operations[0]
            action = operation.get("action")
            if action == "add":
                return self._store.add(
                    self._target, operation.get("content") or "", _lock_held=True,
                )
            if action == "replace":
                return self._store.replace(
                    self._target,
                    operation.get("old_text") or "",
                    operation.get("content") or "",
                    _lock_held=True,
                )
            if action == "remove":
                return self._store.remove(
                    self._target, operation.get("old_text") or "", _lock_held=True,
                )
            return {"success": False, "error": f"Unknown action '{action}'. Use: add, replace, remove"}
        return self._store.apply_batch(self._target, operations, _lock_held=True)

    def entries(self) -> List[str]:
        return list(self._store._entries_for(self._target))

    def read_current_checked(self) -> Tuple[List[str], bool]:
        return MemoryStore._read_entries_checked(self.path)

    def restore(self, entries: List[str]) -> None:
        MemoryStore._write_file(self.path, entries)
        self._store._set_entries(self._target, entries)

    def adopt(self, entries: List[str]) -> None:
        self._store._set_entries(self._target, entries)
