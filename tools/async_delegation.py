...
def _persist_dispatch(record: Dict[str, Any]) -> None:
    ...
    task_payload = {
        key: record.get(key)
        for key in ("goal", "goals", "context", "toolsets", "role", "model", "routes", "is_batch", "task_indexes", *_ROUTING_KEYS)
        if key in record}
    ...
    with _DB_LOCK, _transaction() as conn:
        ...
        for row in rows:
            ...
            event = {
                ...
                "model": task.get("model"), **({"routes": task["routes"]} if task.get("routes") is not None else {}),
...
def dispatch_async_delegation_batch(
    *, goals: List[str], context: Optional[str], toolsets: Optional[List[str]], role: str, model: Optional[str],
    ...
    handle = _dispatch(
        ...
        task_indexes=task_indexes, routes=routes,
        capacity_error=(
            f"Async delegation capacity reached ({max_async_children} running). Wait for one to finish "
