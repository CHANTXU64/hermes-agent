...
class MemoryStore:
    ...
    @staticmethod
    def path_for_target(target: str) -> Path:
    ...
    def preview_entries(entries: List[str], width: int = 80) -> List[str]:
        ...
        return MemoryStore._previews(entries, width)
    ...
    @staticmethod
    def sanitize_entries_for_snapshot(
        entries: List[str], filename: str
    ) -> List[str]:
        """Public Store seam for threat-scanned background-review snapshots."""
        return MemoryStore._sanitize_entries_for_snapshot(entries, filename)
...
class MemoryStoreTransaction:
    ...
    Fork policy can coordinate audit persistence without importing or calling
    any private ``MemoryStore`` implementation details.
    """
...
def memory_tool(
    ...
    if action == "history":
        return json.dumps(
            governance.history(store, target, old_text or ""),
            ensure_ascii=False,
        )
    ...
    if operations:
        ...
        if gate_result is not None:
            return gate_result
        result = governance.apply(store, target, governed_operations)
    ...
    governance_metadata = governance.metadata(operation)
    ...
    result = governance.apply(store, target, [operation])
    return json.dumps(result, ensure_ascii=False)
...
# Compatibility alias accepted by the handler without replacing Governance's
# reason/evidence/history/deletion contract.
...
_memory_properties["operations"]["items"]["properties"]["new_text"] = {
    "type": "string",
