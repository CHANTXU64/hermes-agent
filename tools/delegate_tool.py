...
def _build_child_agent(
    ...
    override_reasoning_config: Any = None,
    ...
    if (
        override_reasoning_config is not None
        and override_reasoning_config is not delegation_routing.INHERIT_REASONING
    ):
        rt["reasoning_config"] = (
            dict(override_reasoning_config)
            if isinstance(override_reasoning_config, dict)
            else override_reasoning_config
        )
...
def _build_children(
    task_list: List[Dict[str, Any]], task_schemas: List[Optional[Dict[str, Any]]], routes: list, *,
...
def delegate_task(
    ...
    if normalized_action in _CONTROL_ACTIONS:
        ...
        unexpected = [name for name, value in spawn_fields.items() if value is not None]
        if unexpected:
            return tool_error(
                f"action='{normalized_action}' does not accept spawn route fields: "
                + ", ".join(unexpected)
            )
    ...
    if err:
        return tool_error(err)
    try:
        routes = delegation_routing.resolve_delegation_task_routes(
            cfg=routing_cfg,
        ...
        return delegation_routing.delegation_route_tool_error(exc, tool_error_factory=tool_error)
    creds = routes[0].credentials if routes else {}
            ...
            "tasks": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "provider": _p("string", "Per-task provider override. Pair with 'model' to choose an exact cross-provider route."),
                        "model": _p("string", "Per-task model override."),
                        "reasoning_effort": _p(
                            "string",
                            "Per-task reasoning-strength override. If omitted, the target model's automatic/default reasoning is used.",
                        ),
                    },
                    "required": ["goal"],
                },
...
registry.register(
    ...
    handler=lambda args, **kw: delegate_task(
        ...
        provider=args.get("provider"), model=args.get("model"), reasoning_effort=args.get("reasoning_effort"),
    ),
