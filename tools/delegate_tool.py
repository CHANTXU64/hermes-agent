...
from fork_features import delegation_routing
...
def _build_child_agent(
    ...
    override_reasoning_config: Any = delegation_routing.INHERIT_REASONING,
    ...
    if override_reasoning_config is not delegation_routing.INHERIT_REASONING:
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        for entry in results:
            ...
            for key, value in delegation_routing.child_route_metadata(child).items():
...
def delegate_task(
    ...
    # Fork route policy resolves every task before child construction, so one
    # invalid route cannot leave a partially spawned batch. The generic host
    # credential resolver is injected to keep provider/runtime plumbing here.
    try:
        task_execution = delegation_routing.resolve_delegation_task_routes(
            cfg=cfg,
            ...
            model=model,
            reasoning_effort=reasoning_effort,
            credential_resolver=_resolve_delegation_credentials,
        )
    except delegation_routing.DelegationTaskRouteError as exc:
        return delegation_routing.delegation_route_tool_error(
            exc, tool_error_factory=tool_error
        )
    ...
    for i, t in enumerate(task_list):
        task_route = task_execution[i]
        task_creds = task_route.credentials
        task_reasoning_override = task_route.reasoning_config
    ...
    if background:
        ...
        _routes = [delegation_routing.child_route_metadata(child) for child in _child_agents]
        _route_models = {
