...
def delegate_task(
    ...
    try:
        route_cfg = credentials_cfg if credentials_cfg is not None else cfg
    ...
    # A heterogeneous batch has no single truthful provider/model header.
    ...
    for i, t in enumerate(task_list):
        ...
        try:
            ...
            # Explicit-pin preflight failures refuse the spawn loudly.
...
DELEGATE_TASK_SCHEMA = {
    "name": "delegate_task",
    # NOTE: description / tasks.description are placeholder values. The real
    ...
    "parameters": {
        ...
        "properties": {
            ...
            # (object), plus Fork routing fields `provider` / `model` /
            # `reasoning_effort` / `role` — wrapped into a one-entry batch at
            # dispatch. Legacy, unadvertised (old transcripts/callers only);
            # tasks=[...] is the only advertised spawn shape. Do not re-add
            # these to the schema.
            "tasks": {
                "type": "array",
                "minItems": 1,
                "items": {
                    ...
                    "properties": {
                        ...
                        "context": {
                            "type": "string",
                            "description": (
                                "Background THIS child needs: file paths, error "
                                "messages, constraints. Each child sees only its "
                                "own context — repeat shared background in every "
                                "task that needs it."
                            ),
                        },
                        "provider": {
                            "type": "string",
                            "description": (
                                "Per-task provider override. Pair with "
                                "'model' to choose an exact cross-provider "
                                "route. If 'model' is supplied without this "
                                "field, Hermes infers the provider when exactly "
                                "one configured provider exposes it."
                            ),
                        },
                        "model": {
                            "type": "string",
                            "description": (
                                "Per-task model override. With no 'provider', "
                                "Hermes searches configured providers with "
                                "usable credentials and uses a unique match; "
                                "ambiguous or missing matches return guidance "
                                "instead of guessing."
                            ),
                        },
                        "reasoning_effort": {
                            ...
                            "description": (
                                "Per-task reasoning-strength override. Hermes "
                                "uses it when the selected provider/model "
                                "request path can apply the exact value; "
                                "otherwise that same route uses its normal "
                                "automatic/default reasoning configuration."
                            ),
                        },
                        "output_schema": {
                            "type": "object",
