...
from tools.approval_context import (
    ...
    _get_approval_config, _get_approval_mode,
)
...
def _automated_denial_key() -> Optional[tuple[str, str]]:
    """Scope repeat-to-human state to one session and one real agent turn."""
...
def _request_repeat_manual_approval(
    ...
    *,
    ...
    """Route the second similar automated denial to one fresh human decision."""
    ...
    digest = hashlib.sha256(
        f"{source_kind}\0{action}".encode("utf-8")
    ).hexdigest()[:20]
    result = _run_approval_gate(
        ...
        single_query_deny_message=(
            "BLOCKED: Repeated operations cannot request one-shot human "
            "approval in single-query mode (-q)."
        ),
...
def clear_session(session_key: str) -> None:
    ...
    for entry in entries:
        ...
        entry.event.set()
    from fork_features.approval.policy import clear_retry_session
    clear_retry_session(session_key, lock=_lock)
    _release_permission_mode_dependents(session_key)
...
def _human_decision(spec: _GateSpec, *, command: str, description: str,
                    ...
                    cwd: Optional[str] = None, source_kind: str = "shell",
                    read_script: Optional[Callable[[str], Optional[str]]] = None,
                    script_evidence: Optional[list[dict[str, str]]] = None,
                    one_shot_only: bool = False,
                    denial_source_kind: Optional[str] = None) -> dict:
    ...
    def deny(template: str, outcome: str, **fmt) -> dict:
        ...
        if denial_source_kind is not None or spec in {_COMMAND_GATE, _EXECUTE_CODE_GATE}:
            ...
            message = template.format(description=description, breaker=breaker, **fmt)
        result = _denied(
            message,
    ...
    if spec.transport:
        attempt = _present_with_selected_transport(
            ...
            allow_session=not effective_smart_denied, allow_permanent=allow_permanent,
        )
    ...
    if is_gateway or is_ask:
        ...
        if notify_cb is not None:
            ...
            data = {
                ...
                "allow_permanent": permanent_capable and not effective_smart_denied,
                "allow_session": not effective_smart_denied,
            }
            if effective_smart_denied:
        ...
        if not _should_fall_through_to_cli_approval(
            ...
            if one_shot_only:
                return {
                    ...
                    "status": "blocked",
                    "message": (
                        "BLOCKED: the repeated operation requires one-shot human "
                        "approval, but no live approval callback is available."
                    ),
                    "pattern_key": pattern_key,
                    "description": description,
            ...
            return _pending_result(
                ...
                pattern_keys=pattern_keys, body=pending_body, smart_denied=effective_smart_denied,
            )
    ...
    choice = prompt_dangerous_approval(prompt_command, prompt_description, allow_permanent=allow_permanent,
                                       smart_denied=effective_smart_denied, approval_callback=approval_callback)
...
def _run_approval_gate(
    *, pattern_key: str, description: str, display_target: str, approval_callback=None,
    ...
    one_shot_only: bool = False, denial_source_kind: Optional[str] = None,
) -> dict:
    ...
    if not is_cli and not is_gateway and not is_ask:
        ...
        for ctx in _unattended_contexts():
            ...
            if fail_closed_when_no_human:
                ...
                result = _blocked(no_human_block_message or (
                ...
                if one_shot_only:
                    result.update(outcome="approval_unavailable", user_consent=False)
                return result
    ...
    return _human_decision(
        ...
        one_shot_only=one_shot_only, denial_source_kind=denial_source_kind,
    )
...
def check_all_command_guards(command: str, env_type: str,
    ...
    # A latched user denial outranks a prepared batch decision; the guard is still
    # consumed above so its single-use contract holds either way.
    ...
    if approval_mode == "smart":
        direct_scripts = _fork_approval_policy().collect_script_evidence(
            command, cwd=cwd, source_kind="shell", read_script=read_script,
        )
    ...
    return _human_decision(
        ...
        cwd=cwd, source_kind="shell", read_script=read_script, script_evidence=direct_scripts,
    )
...
def check_execute_code_guard(code: str, env_type: str, has_host_access: bool = False,
    ...
    script_evidence = []
    if approval_mode == "smart":
        script_evidence = _fork_approval_policy().collect_script_evidence(
            code, cwd=cwd, source_kind="python", read_script=read_script,
        )
    return _human_decision(
        ...
        cwd=cwd, source_kind="python", read_script=read_script, script_evidence=script_evidence,
