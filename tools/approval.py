...
def _call_approval_llm(**kwargs: Any) -> Any:
    ...
    logger.debug(
        "Smart approvals: assessing structured risk (timeout=%ss)", timeout
    )
    try:
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Fork Smart-DENY handling returns through the first/repeat denial policy
    # below, so only non-denial or ESCALATE paths reach the shared manual tail.
    # Keep the upstream UI-scope flag defined for those paths.
