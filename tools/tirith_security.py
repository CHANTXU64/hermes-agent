...
def _supports_check_protocol(path: str) -> bool:
    """Return whether *path* implements the Tirith protocol Hermes invokes."""
    try:
        result = subprocess.run(
            [path, "check", "--help"],
            capture_output=True,
    ...
    return result.returncode == 0 and all(
        flag in help_text
        for flag in ("--json", "--shell", "--non-interactive")
    )
...
def _compatible_path_candidate(path: str | None) -> str | None:
    ...
    _warn_once(
        ...
        "ignoring incompatible tirith executable: %s",
        path,
    )
    return None
...
def _find_local_tirith() -> str | None:
    ...
    if found:
        return found
    if _is_executable(hermes_bin):
        return _compatible_path_candidate(hermes_bin)
    return None
