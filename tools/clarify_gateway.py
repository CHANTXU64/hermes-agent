...
def attempt_text_response_for_session(
    ...
    *,
    ...
    if not str(response or "").strip() and response_context and not entry.choices:
        resolved_response = attach_clarify_response_context("", response_context)
        return (
            TEXT_RESOLVED
    ...
    resolved_response = attach_clarify_response_context(coerced, response_context)
    if resolve_gateway_clarify(entry.clarify_id, resolved_response):
