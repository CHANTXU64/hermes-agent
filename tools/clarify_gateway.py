...
def wait_for_response(clarify_id: str, timeout: float) -> Optional[object]:
    ...
    Returns the resolved response, or ``None`` on timeout. Gateway text replies
    with attachment context use ``ClarifyResponsePayload``; ordinary button
    and text replies remain strings.
    """
    with _lock:
...
def resolve_gateway_clarify(clarify_id: str, response: object) -> bool:
    ...
    with _lock:
        ...
        if entry is None or entry.event.is_set():
            return False
        if isinstance(response, ClarifyResponsePayload):
...
def _coerce_multi_select_text(entry: _ClarifyEntry, text: str) -> Optional[str]:
    ...
    for token in tokens:
        ...
        # Exact label match (case-insensitive, presentation suffix ignored).
        matched = None
...
def attempt_text_response_for_session(
    ...
    *,
    ...
    if resolve_gateway_clarify(entry.clarify_id, resolved_response):
...
def resolve_text_response_for_session(
    ...
    *,
    ...
    """
    return attempt_text_response_for_session(
        session_key,
        response,
