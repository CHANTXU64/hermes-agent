...
def _fabricated_entry(idx: int, status: str, error: str, child: Any, duration: float = 0) -> Dict[str, Any]:
    ...
    from fork_features.delegation_routing import child_route_metadata
...
def _build_result_entry(
    ...
    # Safe effective route metadata must reflect the child that actually ran,
    # not only the requested route. This survives provider/model normalization
    # and records the effective reasoning level without credentials.
    from fork_features.delegation_routing import child_route_metadata

    entry.update(child_route_metadata(child))
