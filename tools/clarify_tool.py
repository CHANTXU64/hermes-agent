...
class ClarifyResponsePayload:
    """Canonical clarify response plus separate attachment context."""

    user_response: str
    response_context: str


def _unwrap_clarify_response(raw) -> Tuple[object, Optional[str]]:
    if isinstance(raw, ClarifyResponsePayload):
        context = str(raw.response_context or "").strip() or None
        return raw.user_response, context
    return raw, None
...
def _invoke_callback(callback, question, choices, multi_select, recommended_index=None):
    """Invoke a platform callback without mixing display metadata into choice values."""
    kwargs = {}
    if _accepts_kwarg(callback, "multi_select"):
        kwargs["multi_select"] = multi_select
    if recommended_index is not None and _accepts_kwarg(callback, "recommended_index"):
        kwargs["recommended_index"] = recommended_index
...
def _normalize_questions(questions) -> tuple:
    ...
    for index, item in enumerate(questions):
        ...
        normalized.append({
            ...
            "recommended_index": 0 if choices and len(choices) >= 2 else None,
...
def _batch_result(normalized: List[dict], answers: dict, timed_out: bool, notice: Optional[str] = None) -> str:
    ...
    for entry in normalized:
        ...
        raw, response_context = _unwrap_clarify_response(raw)
        row = {
        ...
        if response_context is not None:
            row["response_context"] = response_context
...
def clarify_tool(question: str, choices: Optional[List[str]] = None, multi_select: bool = False,
    ...
    if callback is None:
        return tool_error(_UNAVAILABLE)
    # Choice values remain canonical; recommendation is separate display metadata.
    recommended_index = 0 if choices is not None and len(choices) >= 2 else None
    try:
        raw_response = _invoke_callback(
            callback, question, choices, multi_select, recommended_index
        )
    except Exception as exc:
        return tool_error(f"Failed to get user input: {exc}")
    raw_response, response_context = _unwrap_clarify_response(raw_response)
    ...
    if response_context is not None:
        result["response_context"] = response_context
    return json.dumps(result, ensure_ascii=False)
