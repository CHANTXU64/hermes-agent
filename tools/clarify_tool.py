...
# Canonical timeout sentinel returned to the agent when the user never answers.
TIMEOUT_RESPONSE = (
    "The user did not provide a response within the time limit. "
    "Use your best judgement to make the choice and proceed."
)

# Presentation suffix shared by every clarify surface.
...
def _batch_result(normalized: List[dict], answers: dict, timed_out: bool) -> str:
    ...
    for entry in normalized:
        ...
        if isinstance(raw, ClarifyResponsePayload):
            response_context = str(raw.response_context or "").strip() or None
            raw = raw.user_response
...
def clarify_tool(
    ...
    response_context = None
    if isinstance(raw_response, ClarifyResponsePayload):
        response_context = str(raw_response.response_context or "").strip() or None
