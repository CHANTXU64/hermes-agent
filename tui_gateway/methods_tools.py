...
def _cmd_undo(rid, params, session, name, arg):
    ...
    # Notify memory providers exactly once; the dedicated rewind hook both
    # truncates provider state and invalidates cached per-turn context.
    agent = session.get("agent")
    if agent is not None:
        # See #6672 + #21910.
        mm = getattr(agent, "_memory_manager", None)
        for step in (
            lambda: mm is not None and mm.on_session_rewind(
                session_key, turns_undone=turns_undone),
