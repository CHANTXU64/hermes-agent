...
def _clarify_block(
    sid: str, q, c, multi_select=False, questions=None, recommended_index=None,
) -> str:
    ...
    Fork: a ``recommended_index`` of 0 marks that choice before it goes on the wire."""
    from tools.clarify_tool import mark_recommended
    from tui_gateway import server_requests
    if questions:
        wire = []
        for e in questions:
            choices = e["choices"]
            if choices and e.get("recommended_index") == 0:
                choices = mark_recommended(list(choices))
            wire.append({"qid": e["qid"], "question": e["question"], "choices": choices,
    ...
    if c and recommended_index == 0:
        c = mark_recommended(list(c))
