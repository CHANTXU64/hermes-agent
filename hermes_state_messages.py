...
class SessionMessagesMixin:
    ...
    def replace_messages(self, session_id: str, messages: List[Dict[str, Any]], active_only: bool = False,
        ...
        # Rewrites (/undo, /retry, compaction, gateway transcript repair) carry
        # durable conversation content only. Legacy request sidecars may remain in
        # old rows for schema compatibility, but must not be copied into fresh rows.
    ...
    def _fetch_conversation_rows(self, session_ids: List[str], active_clause: str, *, with_session_id: bool,
                                 order_by: str = "id"):
        """``_CONVERSATION_ROW_COLUMNS`` rows for *session_ids*."""
        ...
        return self._read_all(
            ...
            f"{active_clause} ORDER BY {order_clause}", tuple(session_ids))
    ...
    def get_messages_as_conversation(self, session_id: str, include_ancestors: bool = False,
                                     ...
                                     include_timestamps: bool = True, order_by: str = "id",
    ...
    def _rows_to_conversation(self, rows, *, session_id: str, include_ancestors: bool, repair_alternation: bool,
        ...
        for row in rows:
            ...
            if include_timestamps:
                msg.update((col, row[col]) for col in ("timestamp",) if row[col])
            msg.update(
