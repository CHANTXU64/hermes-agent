...
VALID_HOOKS: Set[str] = {
    ...
    "on_session_finalize", "on_session_reset", "on_session_reset_complete",
...
SHELL_UNSUPPORTED_HOOKS: Set[str] = {"transform_api_error_classification", "on_compression_prepare_commit"}
...
class PluginContext:
    ...
    def request_fork(self) -> Any:
        return self._manager.request_fork
...
def get_plugin_auxiliary_tasks() -> List[Dict[str, Any]]:
    ...
    entries = dict(manager._aux_tasks)
    try:
        memory_plugins = importlib.import_module("plugins.memory")

        for entry in memory_plugins.get_memory_provider_auxiliary_tasks():
            key = entry.get("key") if isinstance(entry, dict) else None
            if key:
                entries.setdefault(key, entry)
    except Exception:
        logger.debug("Active memory-provider auxiliary task discovery failed", exc_info=True)
