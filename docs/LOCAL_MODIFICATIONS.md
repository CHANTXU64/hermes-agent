...
- Every macOS Gateway LaunchAgent generated by Hermes receives a per-process
  open-file soft limit of 8192, including the default profile, named profiles,
  service repair, and future installs.

What changed:

- `generate_launchd_plist()` now emits
  `SoftResourceLimits.NumberOfFiles = 8192` in the shared launchd XML template.
- The setting is scoped to each generated Hermes Gateway LaunchAgent and does
  not change the user's global launchd limit.

Why it matters:

- macOS otherwise starts the long-running Gateway with a soft limit of 256.
  A busy process can then fail ordinary state, lock, and network operations with
  `Too many open files` even while launchd still reports the service as running.
- Keeping the limit in the generator prevents `gateway start`, service repair,
  reinstall, or creation of a named profile from deleting a manual plist edit.

Merge protection:

- Preserve when: the generated macOS Gateway service can still inherit a 256
  open-file soft limit under normal launchd startup.
- Drop when: upstream provides an equivalent profile-safe generated resource
  limit or a stronger bounded file-descriptor lifecycle guarantee that makes the
  higher service limit unnecessary.
- Ask user when: a replacement changes the limit globally instead of scoping it
  to Hermes Gateway LaunchAgents.

Verification:

```bash
./venv/bin/python -m pytest tests/fork/test_launchd_open_file_limit.py tests/gateway/test_status.py::TestLaunchdPlistRespawnGovernance -q -o 'addopts='
./venv/bin/python -m py_compile hermes_cli/gateway.py tests/fork/test_launchd_open_file_limit.py
git diff --check
```

Feature docs: none — this is one generated service key with a focused parsed-plist regression.
...
- Self-contained Clarify decision cards:
  ...
  - `tests/tools/test_clarify_tool.py`
  - `docs/LOCAL_MODIFICATIONS.md`
- Launchd Gateway open-file ceiling:
  - `hermes_cli/gateway.py`
  - `tests/fork/test_launchd_open_file_limit.py`
  - `docs/LOCAL_MODIFICATIONS.md`
