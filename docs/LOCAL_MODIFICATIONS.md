...
- 2026-07-20 tool-budget notice correction: transcript rebuild and clean-on-retain drop the exact Hermes runtime notification `You've reached the maximum number of tool-calling iterations allowed. ... without calling any more tools.`. When the real user request is still open, its following visible final answer closes that request; when the preceding turn already has an answer, a following visible response remains an assistant-only event. If Hermes prefixes the exact notification line to real user text, only that line is removed and the residual user text is retained. Matching is limited to the complete canonical line, so same-line user-authored text that extends or discusses the sentence remains ordinary conversation content.
...
- 2026-07-20 tool-budget notice correction: the focused transcript regression first failed because the injected notice became a retained user turn, then passed after exact canonical-line filtering and assistant-output preservation. A second production-shaped RED case proved that a leading notice could contaminate residual real user text; after line-level stripping, real StateDB message `105443` cleans to exactly `继续` with both the notice and task-list rehydration block absent. The related Hindsight suite passed with `254 passed`; `python -m py_compile` and `git diff --check` passed.
