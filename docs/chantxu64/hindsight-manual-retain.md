...
- Slack native slash slots under the 50-command cap preserve canonical `/debug` despite fork-only `/retain`; low-frequency `/credits`, `/insights`, `/disk_cleanup`, and `/lcm` remain reachable through `/hermes <command>` on Slack.
