...
# Auxiliary tasks are discovered lazily for the configured memory provider.
# ``provider -> entries`` also acts as a negative cache for providers with none.
...
class _ProviderCollector:
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        """Capture a memory provider's auxiliary task outside general discovery."""
        if not isinstance(key, str) or not key or not all(
            char.isalnum() or char == "_" for char in key
        ):
            raise ValueError(f"Invalid memory auxiliary task key: {key!r}")
        entry = {
            "key": key,
            ...
            "plugin": f"memory:{self.name}",
            "plugin_key": f"memory:{self.name}",
        }
        entries = _MEMORY_AUXILIARY_TASKS.setdefault(self.name, [])
...
def get_memory_provider_auxiliary_tasks() -> List[dict]:
    """Auxiliary tasks from the configured memory provider, loaded at most once."""
    active_provider = _get_active_memory_provider()
    if not active_provider:
        return []
    if active_provider not in _MEMORY_AUXILIARY_TASKS:
        # Seed the negative cache before loading to prevent recursive discovery.
