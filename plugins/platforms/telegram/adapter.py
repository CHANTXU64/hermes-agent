...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _send_chunks(
        ...
        metadata: Optional[Dict[str, Any]], error_types: tuple, *, plain_text: bool = False) -> SendResult:
    ...
    async def edit_message(
        ...
        if finalize and not bool((metadata or {}).get("plain_text")) and self._rich_eligible(content):
    ...
    def _truncate_plain_text_literal(self, content: str) -> list[str]:
        """Split literal text by Telegram's UTF-16 limit without Markdown fence repair."""
        if utf16_len(content) <= self.MAX_MESSAGE_LENGTH:
            return [content]
        indicator_reserve = 16
        payload_limit = max(1, self.MAX_MESSAGE_LENGTH - indicator_reserve)
        raw_chunks: list[str] = []
        current: list[str] = []
        current_units = 0
        for char in content:
            units = utf16_len(char)
            if current and current_units + units > payload_limit:
    ...
    async def _send_overflow_continuation(
        ...
        plain_text = bool((metadata or {}).get("plain_text"))
        for use_markdown in ((False,) if plain_text else ((True, False) if finalize else (False,))):
            try:
                if use_markdown:
                    ...
                    text = chunk if plain_text or not finalize else _strip_mdv2(chunk)
                return await _await_with_thread_deadline(
                    self._bot.send_message(
                ...
                if "reply message not found" in str(send_err).lower():
                    ...
                    try:
                        return await _await_with_thread_deadline(
                            ...
                            chat_id=normalize_telegram_chat_id(chat_id),
                            text=chunk if plain_text or not finalize else _strip_mdv2(chunk),
                            parse_mode=None, **retry_thread_kwargs, **base),
                            timeout=_TEXT_SEND_DEADLINE, label="telegram-send", dump_on_blocked_loop=False)
    ...
    async def _edit_overflow_split(
        ...
        try:
            if finalize and bool((metadata or {}).get("plain_text")):
                await self._bot.edit_message_text(
                    chat_id=normalize_telegram_chat_id(chat_id),
                    message_id=int(message_id), text=first_chunk, parse_mode=None,
                )
            elif finalize:
                await self._edit_markdown_or_plain(
