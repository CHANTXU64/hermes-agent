            ...
            for _msg_idx in range(_scan_start, len(messages)):
                ...
                _batch_rows.append({
                    ...
                    "platform_message_id": str(
                        msg.get("platform_message_id") or msg.get("message_id") or ""
                    ),
                    "timestamp": _row_timestamp,
                    # Request-only provider context is intentionally omitted.
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        try:
            ...
            for msg in messages:
                ...
                if _is_ephemeral_scaffolding(msg):
                    continue
                # Provider-only context may still be needed by a live redirect
                # when an early crash-resilience snapshot runs. Sanitize a copy
                # so persistence never consumes or records that same-turn state.
                msg = dict(msg)
                msg.pop("api_content", None)
                msg.pop("_request_only_api_content", None)
