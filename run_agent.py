...
class AIAgent(
        ...
        *,
        ...
        try:
            sync_kwargs = {
                ...
                **({"messages": memory_messages} if memory_messages is not None else {}),
            }
            ...
            if not is_trivial_prompt(user_text):
                ...
                params = inspect.signature(queue_prefetch).parameters
                if "turn_id" in params or any(
                    param.kind is inspect.Parameter.VAR_KEYWORD
                    for param in params.values()
                ):
                    queue_kwargs["turn_id"] = (
                        turn_id or getattr(self, "_current_turn_id", "") or ""
                    )
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        # Top-level MODEL spawns run in the background (handle returned, results re-enter as messages).
        # Control actions stay synchronous; an ORCHESTRATOR SUBAGENT (depth > 0) also stays synchronous —
        # it needs worker results in-turn and owns no gateway session. The model-supplied `background`
