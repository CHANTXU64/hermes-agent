...
@pytest.mark.parametrize(
    ...
    [(50, 1, False), (150, 1, False), (50, 1, True)],
    ids=[
        "pressure-cleared-anchored-no-recompaction",
        "pressure-still-high-stays-capped",
        "pressure-cleared-after-provider-recovery-stays-anchored",
    ],
)
def test_pre_api_compression_budget_rearms_only_after_pressure_clears(
    ...
    decision, so every pressure-cleared case performs exactly ONE compaction
    (the pre-anchor one). A malformed primary response does not create an
    anchor, but the first valid fallback response does; its exact prompt count
    then governs the next tool-result turn instead of the stale rough estimate.
    """
    with (
