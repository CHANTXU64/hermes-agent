...
class TestSessionJsonSnapshotOptIn:
    ...
    def test_save_session_log_strips_request_only_sidecars(self, agent, tmp_path):
        agent._session_json_enabled = True
        agent.logs_dir = tmp_path
        messages = [
            {
        ...
        assert messages[0]["api_content"] == "provider-only redirect scaffold"
...
class TestBuildSystemPrompt:
    ...
    def test_profile_guidance_when_only_user_profile_enabled(
        ...
        """
        from agent.prompt_builder import USER_PROFILE_GUIDANCE
        ...
        assert (
            "The built-in notes store is disabled; use only target='user' "
            "for built-in memory operations."
        ) in prompt
        assert "use only target='memory'" not in prompt
