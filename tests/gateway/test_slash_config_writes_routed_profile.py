...
async def test_memory_and_skills_review_commands_use_routed_profile_from_dispatch(homes):
    ...
    with _profile_runtime_scope(routed_home):
        # Fork memory governance: a durable write carries its reason and evidence.
        audit = {"reason": "routed approval test", "evidence": "test fixture"}
        memory_approve = wa.stage_write(
            wa.MEMORY, {"action": "add", "target": "memory", "content": "routed approved", **audit},
            summary="routed-memory-approve", origin="foreground")
        memory_reject = wa.stage_write(
            wa.MEMORY, {"action": "add", "target": "memory", "content": "routed rejected", **audit},
