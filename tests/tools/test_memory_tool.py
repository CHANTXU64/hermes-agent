...
class TestMemoryStoreReplace:
    ...
    def test_replace_same_across_single_batch_and_approval_replay(self, tmp_path, monkeypatch):
        ...
        op = {
            "action": "replace", "old_text": "beta fact.",
            "content": "beta fact, updated.",
            "reason": "exercise replace parity", "evidence": "unit test fixture",
        }
        results = {}
...
class TestBackgroundReviewDeleteGate:
    ...
    def test_add_still_allowed_in_background_review(self, store):
        ...
        try:
            result = json.loads(memory_tool(
                ...
                reason="User asked to keep this fact.",
                evidence="Current user statement requesting durable memory.",
                store=store,
            ))
        finally:
            reset_current_write_origin(token)
    ...
    def test_foreground_remove_unaffected(self, store):
        ...
        result = json.loads(memory_tool(
            action="remove",
            old_text="supervised turn",
            reason="User asked to drop this supervised entry.",
            evidence="Current user request in the live turn.",
            deletion_type="safe",
            store=store,
        ))
        assert result["success"] is True
    ...
    def test_attended_review_keeps_full_operation_set(self, store):
        ...
        try:
            result = json.loads(memory_tool(
                action="replace",
                old_text="entry an explicit",
                content="rewritten by refine",
                reason="User requested an explicit refine rewrite.",
                evidence="Attended /refine instruction in the live turn.",
                store=store,
            ))
        finally:
