...
class TestMultiSelectTextFallback:
    ...
    def test_multi_select_keeps_attachment_context_out_of_user_response(self):
        """Media context must not corrupt the canonical multi-select result."""
        import json
        from tools import clarify_gateway as cm
        from tools.clarify_tool import clarify_tool

        self._register_multi(cid="m-context")
        ...
        callback_response = cm.wait_for_response("m-context", timeout=0.1)
        ...
        result = json.loads(
            clarify_tool(
                ...
                choices=["A", "B", "C"],
                multi_select=True,
                callback=lambda question, choices, **kwargs: callback_response,
            )
        )
        assert result["user_response"] == ["A", "C"]
        assert result["response_context"] == context
