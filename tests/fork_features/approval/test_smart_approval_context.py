...
def test_user_denial_message_uses_configured_language(monkeypatch):
    ...
    assert chinese.startswith("已阻止：用户不允许执行这项操作。")
    assert "用户给出的原因：“范围不对”" in chinese
    assert "不要重试、改写、拆分或换用其他路径" in chinese
    assert english.startswith("BLOCKED: The user does not permit this operation.")
...
def test_direct_script_evidence_does_not_follow_local_imports(tmp_path: Path):
    ...
    helper.write_text("VALUE = 'secret implementation detail'\n")
...
def test_terminal_smart_review_reads_direct_script_once(monkeypatch, tmp_path: Path):
    ...
    script.write_text("print('current entry')\n")
    reads = []
    llm_call = {}
    ...
    monkeypatch.setattr(
        "tools.approval.is_current_session_yolo_enabled", lambda: False
    )
    monkeypatch.setattr(
...
def test_unreadable_direct_script_does_not_override_safe_model_decision(tmp_path: Path):
    response = _approval_response(
        decision="approve",
        risk_level="medium",
        authorization="sufficient",
        reason="可见操作属于正常步骤，入口证据不可用本身不构成风险。",
    )
