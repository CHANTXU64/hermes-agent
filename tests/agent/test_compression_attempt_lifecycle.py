...
class TestWorkerTeardownOnCeiling:
    def test_cooperative_worker_joined_within_grace(self):
        ...
        msgs, prompt = run_compress_context_with_progress_timeout(
            ...
            # Force the TOTAL-ceiling path and leave a scheduler-independent
            # grace window for the cooperative unwind under parallel CI load.
    ...
    def test_uninterruptible_worker_is_orphaned_with_lease_retained(self):
        ...
        msgs, prompt = run_compress_context_with_progress_timeout(
            ...
            messages=original,
            system_prompt_fallback="fallback",
            # This case exercises the TOTAL-ceiling lease-retention path.
            # Keep the idle deadline above the total ceiling so host CPU
            # scheduling cannot accidentally turn it into an idle-stall case.
