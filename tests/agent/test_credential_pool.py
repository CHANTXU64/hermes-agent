...
def test_codex_cyclic_selection_ignores_exhaustion_and_persists_current(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    _write_auth_store(
        tmp_path,
        {
                ...
                "openai-codex": [
                    {
                        ...
                        "last_status_at": time.time(),
                        "last_error_code": 429,
                        "last_error_reset_at": time.time() + 7 * 24 * 3600,
                    },
                    {
                        "id": "codex-2",
                        "label": "two",
    ...
    selected = pool.select_cyclic_current()

    assert selected is not None
    assert selected.id == "codex-1"

    # /new reloads the pool and keeps the current/front account instead of
    # skipping it because of old exhausted/reset_at metadata.
    reloaded = load_pool("openai-codex")
    reselected = reloaded.select_cyclic_current()
    assert reselected is not None
    assert reselected.id == "codex-1"


def test_codex_rate_limit_rotation_is_cyclic_not_exhausting(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    _write_auth_store(
        tmp_path,
        {
            ...
            "credential_pool": {
                "openai-codex": [
                    {"id": "codex-1", "label": "one", "auth_type": "oauth", "priority": 0, "source": "manual:device_code", "access_token": "token-1"},
                    {"id": "codex-2", "label": "two", "auth_type": "oauth", "priority": 1, "source": "manual:device_code", "access_token": "token-2"},
                    {"id": "codex-3", "label": "three", "auth_type": "oauth", "priority": 2, "source": "manual:device_code", "access_token": "token-3"},
                ]
            },
        },
    )
    ...
    pool = load_pool("openai-codex")
    assert pool.select_cyclic_current().id == "codex-1"
    assert pool.rotate_cyclic_on_rate_limit(status_code=429, error_context={"reason": "usage_limit_reached"}).id == "codex-2"
    assert pool.rotate_cyclic_on_rate_limit(status_code=429, error_context={"reason": "usage_limit_reached"}).id == "codex-3"
    assert pool.rotate_cyclic_on_rate_limit(status_code=429, error_context={"reason": "usage_limit_reached"}).id == "codex-1"

    reloaded = load_pool("openai-codex")
    entries = reloaded.entries()
    assert [entry.id for entry in entries] == ["codex-1", "codex-2", "codex-3"]
    assert all(entry.last_status != "exhausted" for entry in entries)


def test_codex_pool_aligns_current_to_runtime_key(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    _write_auth_store(
        tmp_path,
        {
    ...
    pool = load_pool("openai-codex")
    aligned = pool.align_current_to_runtime_key("token-3")

    assert aligned is not None
    assert aligned.id == "codex-3"
    assert [entry.id for entry in pool.entries()] == ["codex-3", "codex-1", "codex-2"]

    reloaded = load_pool("openai-codex")
    assert [entry.id for entry in reloaded.entries()] == ["codex-3", "codex-1", "codex-2"]
    assert reloaded.align_current_to_runtime_key("missing") is None
