...
def test_recover_uses_actual_runtime_key_when_round_robin_current_is_stale(tmp_path, monkeypatch):
    ...
    Under round_robin, select() persists rotated priorities. If the pool is later
    reloaded with current() empty, falling back to current() or select() marks the
    *next* credential rather than the one whose request just failed.
    """
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    monkeypatch.setattr(
        "hermes_cli.auth._import_codex_cli_tokens",
