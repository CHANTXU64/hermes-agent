...
class TestCodexBuildKwargs:
    ...
    def test_codex_cache_scope_headers_normalize_cron_session_id(self, transport):
        ...
        assert first_run["thread-id"] == first_run["x-client-request-id"]
        assert second_run["thread-id"] == second_run["x-client-request-id"]
    ...
    def test_codex_backend_drops_obsolete_session_header_without_session_id(
        ...
        assert headers["thread-id"] == kw["prompt_cache_key"]
        assert headers["x-client-request-id"] == kw["prompt_cache_key"]
    ...
    def test_codex_backend_uses_one_cache_key_for_body_and_headers(self, transport):
        """extra_body cannot silently override the key mirrored by headers."""
        kw = transport.build_kwargs(
            model="gpt-5.4",
            messages=[{"role": "user", "content": "Hi"}],
            tools=[],
            ...
            is_codex_backend=True,
            request_overrides={
                "prompt_cache_key": "logical-cache-key",
                "extra_body": {
        ...
        headers = kw["extra_headers"]
        assert kw["prompt_cache_key"] == "logical-cache-key"
        assert headers["thread-id"] == "logical-cache-key"
        assert headers["x-client-request-id"] == "logical-cache-key"
        assert kw["extra_body"] == {"other_field": 42}

    def test_codex_backend_accepts_extra_body_only_cache_key_override(self, transport):
        kw = transport.build_kwargs(
            model="gpt-5.4",
            messages=[{"role": "user", "content": "Hi"}],
            tools=[],
            ...
            request_overrides={
                "extra_body": {
                    "prompt_cache_key": "extra-body-only-key",
                    "other_field": 42,
                }
            },
        )
        ...
        headers = kw["extra_headers"]
        assert kw["prompt_cache_key"] == "extra-body-only-key"
        assert headers["thread-id"] == "extra-body-only-key"
        assert headers["x-client-request-id"] == "extra-body-only-key"
        assert kw["extra_body"] == {"other_field": 42}

    @pytest.mark.parametrize(
        "invalid_top_level_key",
        [True, 7, {"bad": "type"}, ["bad", "type"]],
    )
    def test_codex_backend_non_string_top_level_key_does_not_shadow_body_key(
        self, transport, invalid_top_level_key
    ):
        kw = transport.build_kwargs(
            model="gpt-5.4",
            messages=[{"role": "user", "content": "Hi"}],
            ...
            is_codex_backend=True,
            request_overrides={
                "prompt_cache_key": invalid_top_level_key,
                "extra_body": {
                    "prompt_cache_key": "valid-body-key",
                    "other_field": 42,
                },
            },
        )
        ...
        headers = kw["extra_headers"]
        assert kw["prompt_cache_key"] == "valid-body-key"
        assert headers["thread-id"] == "valid-body-key"
        assert headers["x-client-request-id"] == "valid-body-key"
        assert kw["extra_body"] == {"other_field": 42}
