...
def test_committed_entry_identity_comes_from_locked_store(
    ...
    # Fork: every durable memory mutation must carry reason + evidence, and a
    # remove must classify its deletion.
    audit = {'reason': 'test', 'evidence': 'test'}
    operations = [
        {**op, **audit, **({'deletion_type': 'safe'} if op['action'] == 'remove' else {})}
