...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def get_messages_as_conversation(
        ...
        By default, messages are returned in AUTOINCREMENT id order (true
        insertion order), not timestamp order. This preserves assistant
        tool-call / tool-response adjacency even when platform timestamps are
        replayed, backfilled, or affected by clock regressions. Pass
        ``order_by='timestamp'`` only for event-time transcript/export views.
        ...
        single request for the rest of the session's life. Inspection/export
        ...
        with self._read_ctx() as conn:
            ...
            rows = conn.execute(
                ...
                # Default/order_by='id': AUTOINCREMENT id (true insertion order),
                # NOT timestamp. append_message stamps rows with time.time(),
                # which is not monotonic (WSL2, NTP steps, VM/laptop sleep
                # resume). A later row can carry an earlier timestamp than its
                # predecessor, and ORDER BY timestamp would then sort an
                # assistant tool_calls row after its tool response, breaking
                # tool-call/response adjacency and triggering an HTTP 400 on
                # replay. Pass order_by='timestamp' explicitly for event-time
                # transcript/export views.
